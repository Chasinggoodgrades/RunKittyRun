export class ProtectionOfAncients {
    ...
    private static RegisterUltimateGain() {
        ...
        KittyReachedLevelSix.AddAction(() => {
            if (HitLevel6.includes(GetTriggerUnit().Owner)) return
            if (GetTriggerUnit().HeroLevel < 6) return
            HitLevel6.push(GetTriggerUnit().Owner)
    ...
    public static SetProtectionOfAncientsLevel(unit: Unit) {
        ...
        let abilityLevel: number =
            heroLevel >= UPGRADE_LEVEL_3_REQUIREMENT ? 3 : heroLevel >= UPGRADE_LEVEL_2_REQUIREMENT ? 2 : 0
        ...
        if (abilityLevel > 0) {
            ...
            if (
                (abilityLevel == 2 && !UpgradeLevel2.includes(player)) ||
                (abilityLevel == 3 && !UpgradeLevel3.includes(player))
            ) {
                player.DisplayTimedTextTo(
                    7.0,
                    "{Colors.COLOR_YELLOW_ORANGE}on: level: Congratulations {heroLevel}! You'upgraded: your: ultimate: to: level: ve {abilityLevel}!|r"
                )
            }

            if (abilityLevel == 2) {
                UpgradeLevel2.push(player)
            } else if (abilityLevel == 3) {
                UpgradeLevel3.push(player)
    ...
    private static EndEffectActions(Player: MapPlayer) {
        ...
        let tempGroup = CreateGroup()! // consider changing this to a static group
