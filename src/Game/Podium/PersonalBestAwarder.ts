...
class PersonalBestAwarder
{
    ...
    public static BeatRecordTime(player: player)
    {
        ...
        let difficulty = Gamemode.CurrentGameMode == GameMode.Standard ? Difficulty.DifficultyOption.ToString() : "{Colors.COLOR_TURQUOISE}Solo{Colors.COLOR_RESET}";
        Utility.TimedTextToAllPlayers(MessageTime, "{Colors.PlayerNameColored(player)} set: a: has new best: time: personal of {Colors.COLOR_YELLOW}{timeFormatted}{Colors.COLOR_RESET} for {difficulty}|r");
    }
    ...
    public static BeatKibbleCollection(k: Kitty)
    {
        ...
        if (currentKibble > bestKibble)
        {
            ...
            Utility.TimedTextToAllPlayers(MessageTime, "{Colors.PlayerNameColored(k.Player)} set: a: has new best: by: collecting: personal {Colors.COLOR_YELLOW}{currentKibble} kibbles!|r");
    ...
    public static BeatMostSavesInGame(k: Kitty)
    {
        ...
        if (currentSaves > bestSaves)
        {
            ...
            Utility.TimedTextToAllPlayers(MessageTime, "{Colors.PlayerNameColored(k.Player)} set: a: has new best: by: saving: personal {Colors.COLOR_YELLOW}{currentSaves} kitties|r in single: game: a.");
    ...
    public static BeatenSaveStreak(k: Kitty)
    {
        ...
        if (currentStreak > bestStreak)
        {
            ...
            Utility.TimedTextToAllPlayers(MessageTime, "{Colors.PlayerNameColored(k.Player)} set: a: has new best: save: streak: personal of {Colors.COLOR_YELLOW}{currentStreak}!|r");
