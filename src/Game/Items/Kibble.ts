export class Kibble extends IDisposable {
    ...
    private static KibblesColors: number[] = KibbleList()
    ...
    public SpawnKibble() {
        ...
        StarFallEffect.setPos(x, y, 0)
        StarFallEffect.PlayAnimation(ANIM_TYPE_BIRTH)
        JackPotIndex = 1
        Item = Item.create(Type, x, y)!
    ...
    private static RandomKibbleType(): number {
        return KibblesColors[GetRandomInt(0, KibblesColors.length - 1)]
    }
    ...
    private static KibblePickupEvents(): trigger {
        ...
        trig.AddAction(() => {
            ...
            if (!KibblesColors.includes(item.TypeId)) return
            KibblePickup(item)
        })
        return trig
    }
    ...
    private static JackpotEffect(kitty: Kitty, kibble: Kibble) {
        ...
        } else Utility.SimpleTimer(0.15, () => JackpotEffect(kitty, kibble))
    }
    ...
    private static IncrementKibble(kibblePicker: Kitty) {
        ...
        for (let player in Globals.ALL_PLAYERS) player.Lumber += 1
