export class RingOfSummoning extends Relic {
    ...
    private SetAbilityData(Unit: Unit) {
        ...
        let cooldown = upgradeLevel >= 1 ? SUMMONING_COOLDOWN - UPGRADE_COOLDOWN_REDUCTION : SUMMONING_COOLDOWN
    ...
    private SacredRingOfSummoning() {
        ...
        while (SummonGroup.First != null && count < numberOfSummons) {
            ...
            if (
                !SummonDeadKitty(summoningKitty, kitty) ||
                !DeathlessKitty(summoningKitty, kitty) ||
                !ChainedKitty(summoningKitty, kitty)
            )
                continue

            // Position adjustments and revival
            kitty.Unit.setPos(summoningKittyUnit.X, summoningKittyUnit.Y)
