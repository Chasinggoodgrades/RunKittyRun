import { Rectangle } from 'w3ts'
import { Regions } from '../AutoGenerated/Regions'

class RegionList {
    public static readonly SpawnRegions: Rectangle[] = [
        Regions.Spawn_Area_01,
        Regions.Spawn_Area_02,
        Regions.Spawn_Area_03,
        Regions.Spawn_Area_04,
        Regions.Spawn_Area_05,
        Regions.Spawn_Area_06,
        Regions.Spawn_Area_07,
        Regions.Spawn_Area_08,
        Regions.Spawn_Area_09,
        Regions.Spawn_Area_10,
        Regions.Spawn_Area_11,
        Regions.Spawn_Area_01,
        Regions.Spawn_Area_02,
        Regions.Spawn_Area_03,
        Regions.Spawn_Area_04,
        Regions.Spawn_Area_05,
        Regions.Spawn_Area_06,
        Regions.Spawn_Area_07,
        Regions.Spawn_Area_08,
        Regions.Spawn_Area_09,
        Regions.Spawn_Area_10,
        Regions.Spawn_Area_11,
        Regions.Spawn_Area_01,
        Regions.Spawn_Area_02,
        Regions.Spawn_Area_03,
    ]

    public static readonly WolfRegions: Rectangle[] = [
        Regions.Wolf_Area_01,
        Regions.Wolf_Area_02,
        Regions.Wolf_Area_03,
        Regions.Wolf_Area_04,
        Regions.Wolf_Area_05,
        Regions.Wolf_Area_06,
        Regions.Wolf_Area_07,
        Regions.Wolf_Area_08,
        Regions.Wolf_Area_09,
        Regions.Wolf_Area_10,
        Regions.Wolf_Area_11,
        Regions.Wolf_Area_12,
        Regions.Wolf_Area_13,
        Regions.Wolf_Area_14,
        Regions.Wolf_Area_15,
        Regions.Wolf_Area_16,
        Regions.Wolf_Area_17,
    ]

    public static readonly SafeZones: Rectangle[] = [
        Regions.safe_Area_00,
        Regions.safe_Area_01,
        Regions.safe_Area_02,
        Regions.safe_Area_03,
        Regions.safe_Area_04,
        Regions.safe_Area_05,
        Regions.safe_Area_06,
        Regions.safe_Area_07,
        Regions.safe_Area_08,
        Regions.safe_Area_09,
        Regions.safe_Area_10,
        Regions.safe_Area_11,
        Regions.safe_Area_12,
        Regions.safe_Area_13,
        Regions.safe_Area_14,
    ]

    public static readonly PathingPoints: Rectangle[] = // Nitros and Progress classes
        [
            Regions.safe_Area_00,
            Regions.safe_Area_01,
            Regions.safe_Area_02,
            Regions.safe_Area_03,
            Regions.safe_Area_04,
            Regions.safe_Area_05,
            Regions.safe_Area_06,
            Regions.safe_Area_07,
            Regions.safe_Area_08,
            Regions.safe_Area_09,
            Regions.safe_Area_10,
            Regions.safe_Area_11,
            Regions.safe_Area_12,
            Regions.safe_Area_13,
            Regions.ProgressPoint1,
            Regions.ProgressPoint2,
            Regions.ProgressPoint3,
            Regions.safe_Area_14,
        ]
}
