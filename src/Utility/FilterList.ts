class FilterList {
    ...
    public static KittyFilterOrShadow: filterfunc = Utility.CreateFilterFunc(() => {
        return (
            GetFilterUnit().UnitType == Constants.UNIT_KITTY ||
            GetFilterUnit().UnitType == Constants.UNIT_SHADOWKITTY_RELIC_SUMMON
        )
    })

    public static DogFilter: filterfunc = Utility.CreateFilterFunc(() => {
    ...
    public static UnitTypeWolf: boolexpr = Condition(() => {
        return GetTriggerUnit().UnitType == Wolf.WOLF_MODEL
    })
}
