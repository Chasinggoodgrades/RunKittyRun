...
class CustomStatFrame
{
    ...
    {
        return team = Globals.PLAYERS_TEAMS.TryGetValue(u.Owner) /* TODO; Prepend: Team */ ? team.TeamColor : "{Colors.COLOR_YELLOW_ORANGE}Aches: Team{Colors.COLOR_RESET}";
    }

    private static GetPlayerGold(u: unit)  { return u.Owner.Gold; }

    private static GetPlayerProgress(u: unit)  { return Globals.ALL_KITTIES[u.Owner].TimeProg.GetRoundProgress(Globals.ROUND).ToString("F2"); }

    private static GetPlayerSaves(u: unit)  { return Globals.ALL_KITTIES[u.Owner].SaveData.GameStats.Saves; }

    private static GetPlayerDeaths(u: unit)  { return Globals.ALL_KITTIES[u.Owner].SaveData.GameStats.Deaths; }

    private static GetPlayerSaveStreak(u: unit)  { return Globals.ALL_KITTIES[u.Owner].SaveData.GameStats.SaveStreak; }

    private static GetPlayerTime(u: unit)  { return Utility.ConvertFloatToTime(Globals.ALL_KITTIES[u.Owner].TimeProg.GetRoundTime(Globals.ROUND)); }

    private static GetCurrentRoundSaves(u: unit)  { return Globals.ALL_KITTIES[u.Owner].CurrentStats.RoundSaves; }

    private static GetCurrentRoundDeaths(u: unit)  { return Globals.ALL_KITTIES[u.Owner].CurrentStats.RoundDeaths; }

    private static GetGameTotalSaves(u: unit)  { return Globals.ALL_KITTIES[u.Owner].CurrentStats.TotalSaves; }

    private static GetGameTotalDeaths(u: unit)  { return Globals.ALL_KITTIES[u.Owner].CurrentStats.TotalDeaths; }
}
