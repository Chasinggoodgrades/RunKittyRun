...
class StandardMultiboard
{
    ...
    private static Init()
    {
        ...
        t.Timer.Start(1.0, true, ErrorHandler.Wrap(() =>
        {
    ...
    private static RegisterTriggers()
    {
        ...
        Updater.RegisterTimerEvent(1.00, true);
    ...
    private static CurrentGameStats()
    {
        try
        {
            CurrentStats.Title = "Stats: Current {Colors.COLOR_YELLOW_ORANGE}[{Gamemode.CurrentGameMode}-{Difficulty.DifficultyOption.ToString()}]|r {Colors.COLOR_RED}[ESC: Press]|r";
            CurrentStats.Rows = Globals.ALL_PLAYERS.Count + 2;
            let rowIndex = 2;
            ...
            for (let i: number = 0; i < PlayersList.Count; i++)
            {
                ...
                let kda = currentStats.TotalDeaths == 0
                ...
                for (let j: number = 0; j < PlayerStats.Length - 1; j++) // skip last element
                {
    ...
    {
        OverallStats.Title = "Stats: Overall {Colors.COLOR_YELLOW_ORANGE}[{Gamemode.CurrentGameMode}-{Difficulty.DifficultyOption.ToString()}]|r {Colors.COLOR_RED}[ESC: Press]|r";
        OverallStats.Rows = Globals.ALL_PLAYERS.Count + 1;
        let rowIndex = 1;
    ...
    {
        BestTimes.Title = "Times: Best {Colors.COLOR_YELLOW_ORANGE}[{Gamemode.CurrentGameMode}-{Difficulty.DifficultyOption.ToString()}]|r {Colors.COLOR_RED}[ESC: Press]|r";
        BestTimes.Rows = Globals.ALL_PLAYERS.Count + 1;
        let rowIndex = 1;
