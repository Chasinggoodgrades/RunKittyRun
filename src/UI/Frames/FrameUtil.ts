...
export const CreateHeaderFrame = (parent: Frame): Frame => {
    const header = blzCreateFrameByType(
        'BACKDROP',
    ...
    const title = blzCreateFrameByType('TEXT', `${parent.getName()}Title`, header, 'ScriptDialogText', 0)
    ...
    const closeButton = blzCreateFrameByType(
        'GLUETEXTBUTTON',
    ...
    // Close Actions
    const closeTrigger = Trigger.create()!
