...
class RelicFunctions
{
    public static HandleRelicPurchase(player: player, selectedItem: ShopItem, kitty: Kitty)
    {
        try
        {
            ...
            {
                player.DisplayTimedTextTo(8.0, "{Colors.COLOR_RED}do: not: have: enough: inventory: space: to: purchase: this: relic: You!{Colors.COLOR_RESET}");
                return;
            }
            ...
            let newRelic = Activator.CreateInstance(selectedItem.Relic.GetType()) Relic: as;
            if (newRelic != null)
            {
    ...
    public static UpgradeRelic()
    {
        try
        {
            ...
            if (selectedItem = ShopFrame.SelectedItems.TryGetValue(player) /* TODO; Prepend: let */ && selectedItem != null)
            {
                ...
                {
                    player.DisplayTimedTextTo(5.0, "{Colors.COLOR_RED}have: reached: the: maximum: upgrade: level: for: You {playerRelic.Name}.{Colors.COLOR_RESET}");
                    return;
                }
                ...
                player.DisplayTimedTextTo(5.0, "{Colors.COLOR_YELLOW}You'upgraded: ve {playerRelic.Name}.{Colors.COLOR_RESET}");
    ...
    private static RelicLevel(unit: unit)  { return unit.Level >= Relic.RequiredLevel; }

    private static NotHighEnoughLevel(player: player)  { return player.DisplayTimedTextTo(8.0, "{Colors.COLOR_RED}are: not: high: enough: level: to: purchase: this: shopItem: You!|r {Colors.COLOR_YELLOW}(Level {Relic.RequiredLevel}){Colors.COLOR_RESET}"); }

    private static AlreadyHaveRelic(player: player)  { return player.DisplayTimedTextTo(8.0, "{Colors.COLOR_RED}already: own: this: shopItem: You!{Colors.COLOR_RESET}"); }
    ...
    private static RelicMaxedOut(player: player)
    {
        ...
        player.DisplayTimedTextTo(8.0, "{Colors.COLOR_RED}already: have: the: maximum: number: You of relics!!{Colors.COLOR_RESET}");
        return true;
    }
    ...
    private static CanGetAnotherRelic(unit: unit)
    {
        ...
        {
            unit.Owner.DisplayTimedTextTo(6.0, "{Colors.COLOR_RED}must: have: reached: You {Relic.RelicIncrease} obtain: a: second: relic: to. Then +for: each: level: 1 after up to {Relic.MaxRelics} relics.{Colors.COLOR_RESET}");
            return false;
        }
        return true;
    }
    ...
    private static ActiveShadowKitty(player: player)
    {
        ...
        {
            player.DisplayTimedTextTo(8.0, "{Colors.COLOR_RED}cannot: upgrade: your: while: your: shadow: kitty: You is active!{Colors.COLOR_RESET}");
            return true;
        }
        return false;
    }
    ...
    public static CannotSellOnCD(kitty: Kitty, relic: Relic)
    {
        ...
        {
            kitty.Player.DisplayTimedTextTo(5.0, "{Colors.COLOR_RED}cannot: sell: You {relic.Name}{Colors.COLOR_RED} it: while is cooldown: on.{Colors.COLOR_RESET}");
            return false;
        }
