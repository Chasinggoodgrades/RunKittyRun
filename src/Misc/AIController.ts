export class AIController {
    ...
    public ResumeAi() {
        ...
        if (claimedKitties.has(this.kitty)) {
    ...
    private MoveKittyToPosition() {
        ...
        let distanceToCurrentCenter = Math.Sqrt(
            Math.Pow(kitty.Unit.X - currentSafezoneCenter.X, 2) + Math.Pow(kitty.unit.y - currentSafezoneCenter.Y, 2)
        )
        ...
        let targetPosition =
            reachedLastProgressZoneCenter && allKittiesAtSameOrHigherSafezone
                ? nextSafezoneCenter
        ...
        for (let circle in Globals.ALL_CIRCLES) {
            ...
            if (!deadKitty.Alive) {
                if (!claimedKitties.has(deadKitty)) {
                ...
                if (this.claimedKitties.has(deadKitty) && claimedKitties[deadKitty] == this.kitty) {
                    if (deadKittyProgressZoneId != currentProgressZoneId) {
                        if (
                            IsInSafeZone(this.kitty.Unit.X, this.kitty.unit.y, currentProgressZoneId) &&
                            reachedLastProgressZoneCenter
                        ) {
                            targetPosition =
                                currentProgressZoneId - 1 >= 0
                                    ? GetCenterPositionInSafezone(Globals.SAFE_ZONES[currentProgressZoneId - 1])
                                    : currentSafezoneCenter
                        } else {
        ...
        for (let i: number = 0; i < wolvesInLane.length; i++) {
            ...
            if (
                IsWithinRadius(
                    ...
                    wolf.Unit.X,
                    wolf.unit.y,
                    wolf.IsWalking ? DODGE_RADIUS : DODGE_RADIUS_STILL
                )
            ) {
                wolvesInRange.push(wolf)
            }
        }
    ...
    private GetCompositeDodgePosition(wolves: Wolf[], forwardDirection: [number, number]) {
        ...
        for (let i: number = 0; i < wolves.length; i++) {
            ...
            let ratio: number =
                ...
                (DODGE_RADIUS - CollisionDetection.DEFAULT_WOLF_COLLISION_RADIUS)
        ...
        if (wallPoints.length == 2) {
            let(angleA, angleB) = AnglesFromCenter(
        ...
        HideAllLightnings()
        for (let i: number = 0; i < mergedIntervals.length; i++) {
        ...
        if (mergedIntervals.length == 0) {
            ...
            let wrapGap: number =
                mergedIntervals[0].Start + 2 * MathF.PI - mergedIntervals[mergedIntervals.length - 1].End
            if (wrapGap > 0) {
                let a = MemoryHandler.getEmptyObject<AngleInterval>()
                a.Start = mergedIntervals[mergedIntervals.length - 1].End
                a.End = mergedIntervals[0].Start + 2 * MathF.PI
                freeGaps.push(a)
            }
            // Gaps between consecutive intervals.
            for (let i: number = 0; i < mergedIntervals.length - 1; i++) {
        ...
        HideAllFreeLightnings()
        for (let i: number = 0; i < freeGaps.length; i++) {
        ...
        return (
            kitty.Unit.X + forwardDirection2.X * DODGE_DISTANCE,
            kitty.unit.y + forwardDirection2.Y * DODGE_DISTANCE
        )
    }
    ...
    private cleanArrays() {
        for (let i: number = 0; i < blockedIntervals.length; i++) {
    ...
    private VisualizeBlockedInterval(interval: AngleInterval) {
        ...
        for (let angle: number = interval.Start; angle < interval.End; angle += step) {
            ...
            if (availableBlockedLightnings.length > 0) {
                freeLightning = availableBlockedLightnings[availableBlockedLightnings.length - 1]
                availableBlockedLightnings.RemoveAt(availableBlockedLightnings.length - 1)
            }
            ...
            usedBlockedLightnings.push(freeLightning)
    ...
    private VisualizeFreeInterval(interval: AngleInterval) {
        ...
        for (let angle: number = interval.Start; angle < interval.End; angle += step) {
            ...
            if (availableClearLightnings.length > 0) {
                freeLightning = availableClearLightnings[availableClearLightnings.length - 1]
                availableClearLightnings.RemoveAt(availableClearLightnings.length - 1)
            }
            ...
            usedClearLightnings.push(freeLightning)
            MoveLightning(freeLightning, false, x1, y1, x2, y2)
        }
    }

    private HideAllLightnings() {
        for (let i: number = 0; i < usedBlockedLightnings.length; i++) {
            let lightning = usedBlockedLightnings[i]
            MoveLightning(lightning, false, 0.0, 0.0, 0.0, 0.0)
            availableBlockedLightnings.push(lightning)
        }
    ...
    private HideAllFreeLightnings() {
        for (let i: number = 0; i < usedClearLightnings.length; i++) {
            let lightning = usedClearLightnings[i]
            MoveLightning(lightning, false, 0.0, 0.0, 0.0, 0.0)
            availableClearLightnings.push(lightning)
        }
    ...
    private UseWindWalkIfAvailable() {
        ...
        if (
            wwLvl == 0 ||
            (wwLvl == 1 && kitty.Unit.mana < 75) ||
            (wwLvl == 2 && kitty.Unit.mana < 60) ||
            (wwLvl == 3 && kitty.Unit.mana < 45)
        ) {
            return
        }
