...
class Slider
{
    ...
    public ResumeSlider(isRevive: boolean)
    {
        ...
        if (isRevive)
        {
            ...
            this.ForcedSlideTimer.Start(1.4, false, ErrorHandler.Wrap(() =>
            {
                this.forcedSlideSpeed = null;

                this.ForcedSlideTimer.Start(0.6, false, ErrorHandler.Wrap(() =>
                {
    ...
    {
        let slideSpeed: number = this.forcedSlideSpeed ?? this.absoluteSlideSpeed ?? ((this.kitty.IsMirror ? -1 : 1) * GetUnitMoveSpeed(kitty.Unit));
    ...
    private escaperTurnForOnePeriod()
    {
        ...
        if (remainingDegrees != 0)
        {
            ...
            if (diffToApplyAbs > 0.05)
            {
                ...
                let maxIncreaseRotationSpeedPerPeriod: number = Math.Abs(maxSlideTurnPerPeriod * SLIDE_INTERVAL / rotationTimeForMaximumSpeed);
