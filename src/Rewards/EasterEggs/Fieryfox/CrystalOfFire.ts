...
export class CrystalOfFire {
    ...
    private static RegisterTurnInFiery(): Trigger {
        ...
        Trigger.registerUnitInRage(SpawnChampions.Fieryfox2023, TurnInRange, null)
    ...
    private static RegisterTurnInFandF(): Trigger {
        let Trigger = Trigger.create()!
        Trigger.registerUnitInRage(SpawnChampions.FandF2023, TurnInRange, null)
    ...
    private static FieryfoxEvent() {
        ...
        if (GetDeathAttempts(player) >= 0) {
            ...
            QuestEligible.push(player)
            // 30 seconds to accept the quest.
            Utility.SimpleTimer(30.0, () => {
                if (QuestEligible.includes(player)) QuestEligible.Remove(player)
            })
    ...
    private static AcceptedQuest() {
        ...
        if (!QuestEligible.includes(player)) return
        let kitty = Globals.ALL_KITTIES.get(player)!
        let region = Regions.Spawn_Area_05.Center
        kitty.Unit.setPos(region.x, region.y)
