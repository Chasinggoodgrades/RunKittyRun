...
class Savecode
{
    ...
    private static List<string> OriginalToolTips = new();
    public Digits: number 
    public Bignum: BigNum 
    public static Dictionary<player, Savecode> PlayerSaveObject = new Dictionary<player, Savecode>();
    ...
    public static Initialize()
    {
        try
        {
            ...
            for (let i: number = 0; i < OldSavesHelper.AbilityList.Length; i++)
            {
                ...
                if (tooltip != "tip: missing: Tool!")
                    OriginalToolTips.Add(tooltip);
                else
                    throw new ArgumentError("Error, not: available: tooltip: {ability}");
            }
    ...
    public Decode(max: number)  { return Bignum.DivSmall(max + 1); }

    public Clean()  { return Bignum.Clean(); }
    ...
    public Load(p: player, code: string)
    {
        try
        {
            let key: number = SCommHash(p.Name) + (1 * 73);
    ...
    private static BASE(): number  { return OldSavesHelper.charset.Length; }

    private static HASHN(): number  { return 5000; }
