public class Deathless
{
    public int NormalDeathless1 { get; set; }
    public int NormalDeathless2 { get; set; }
    public int NormalDeathless3 { get; set; }
    public int NormalDeathless4 { get; set; }
    public int NormalDeathless5 { get; set; }
    public int HardDeathless1 { get; set; } = -1;
    public int HardDeathless2 { get; set; } = -1;
    public int HardDeathless3 { get; set; } = -1;
    public int HardDeathless4 { get; set; } = -1;
    public int HardDeathless5 { get; set; } = -1;
    public int ImpossibleDeathless1 { get; set; } = -1;
    public int ImpossibleDeathless2 { get; set; } = -1;
    public int ImpossibleDeathless3 { get; set; } = -1;
    public int ImpossibleDeathless4 { get; set; } = -1;
    public int ImpossibleDeathless5 { get; set; } = -1;
    public int NormalTeamDeathless { get; set; }
    public int HardTeamDeathless { get; set; }
    public int ImpossibleTeamDeathless { get; set; }
}
